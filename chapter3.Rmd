...
In the joined data, m means math and p means portuguese. The list of variables and explanations are following. For some reason, in our joined data some m and p variables from data combination were left in our data. 
...
I think I will choose following variables: 
famrel = quality of family relationships -> Because I think bad relationships lead to alcohol consumption
health = I don't think which way this will be?
activities = if you have ec-activities, you have no time to drink
absences = I have no clue why you have left this as absences p and m, because they are identical, what is the point?. However, I will think if you have hangover, you don't attend school-> thus more absences. 
...
Lets explore our chosen variables and lets make a subtable of those: 
```{r}
...
Health seems not to correlate alcohol use and is not normally distributed (which is natural because most of use are really healthy). Activities don't have the correlation either for the alcohol use. Hoever, absences seem to correlate stongly to alcohol usage. 
...
Lets create also probabilities and prediction variables that will show us how good our model is: 
...
ggplot(joinedS, aes(high_use, probabilities, col=Prediction ))+
  geom_point()
```
And now, I did not check how this was done in datacamp, but lets count how many points were missclassified (the training error): 
```{r}
...
Well, approximately 1/3 of the values was misquessed -> This is better than simple quessing, because if we have to quess 370 times and only 104 times uncorrect, it is very unlikely that we obtain this good values. 
